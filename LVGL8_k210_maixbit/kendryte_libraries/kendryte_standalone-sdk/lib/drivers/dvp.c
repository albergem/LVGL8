...
static void mdelay(uint32_t ms) {
  ...
  while (ms && ms--) {
    for (i = 0; i < 25000; i++) __asm__ __volatile__("nop");
  }
}
...
uint32_t dvp_sccb_set_clk_rate(uint32_t clk_rate) {
  ...
  tmp = dvp->sccb_cfg & (~(DVP_SCCB_SCL_LCNT_MASK | DVP_SCCB_SCL_HCNT_MASK));
  tmp |=
      DVP_SCCB_SCL_LCNT(v_period_clk_cnt) | DVP_SCCB_SCL_HCNT(v_period_clk_cnt);
  dvp->sccb_cfg = tmp;
...
void dvp_sccb_send_data(uint8_t dev_addr, uint16_t reg_addr, uint8_t reg_data) {
  ...
  (g_sccb_reg_len == 8) ? (tmp |= DVP_SCCB_BYTE_NUM_3)
                        : (tmp |= DVP_SCCB_BYTE_NUM_4);

  dvp->sccb_cfg = tmp;

  if (g_sccb_reg_len == 8) {
    dvp->sccb_ctl =
        DVP_SCCB_WRITE_DATA_ENABLE | DVP_SCCB_DEVICE_ADDRESS(dev_addr) |
        DVP_SCCB_REG_ADDRESS(reg_addr) | DVP_SCCB_WDATA_BYTE0(reg_data);
  } else {
    dvp->sccb_ctl =
        DVP_SCCB_WRITE_DATA_ENABLE | DVP_SCCB_DEVICE_ADDRESS(dev_addr) |
        DVP_SCCB_REG_ADDRESS(reg_addr >> 8) |
        DVP_SCCB_WDATA_BYTE0(reg_addr & 0xff) | DVP_SCCB_WDATA_BYTE1(reg_data);
  }
...
uint8_t dvp_sccb_receive_data(uint8_t dev_addr, uint16_t reg_addr) {
  ...
  if (g_sccb_reg_len == 8) {
    dvp->sccb_ctl = DVP_SCCB_WRITE_DATA_ENABLE |
                    DVP_SCCB_DEVICE_ADDRESS(dev_addr) |
                    DVP_SCCB_REG_ADDRESS(reg_addr);
  } else {
    dvp->sccb_ctl = DVP_SCCB_WRITE_DATA_ENABLE |
                    DVP_SCCB_DEVICE_ADDRESS(dev_addr) |
                    DVP_SCCB_REG_ADDRESS(reg_addr >> 8) |
                    DVP_SCCB_WDATA_BYTE0(reg_addr & 0xff);
  }
  dvp_sccb_start_transfer();
...
void dvp_set_display_addr(uint32_t addr) { dvp->rgb_addr = addr; }
...
void dvp_start_convert(void) { dvp->sts = DVP_STS_DVP_EN | DVP_STS_DVP_EN_WE; }
...
void dvp_get_image(void) {
  ...
  while (!(dvp->sts & DVP_STS_FRAME_START))
    ;
  dvp->sts = DVP_STS_FRAME_FINISH | DVP_STS_FRAME_FINISH_WE |
             DVP_STS_FRAME_START | DVP_STS_FRAME_START_WE | DVP_STS_DVP_EN |
             DVP_STS_DVP_EN_WE;
  while (!(dvp->sts & DVP_STS_FRAME_FINISH))
    ;
}
...
void dvp_enable_auto(void) { dvp->dvp_cfg |= DVP_CFG_AUTO_ENABLE; }

void dvp_disable_auto(void) { dvp->dvp_cfg &= (~DVP_CFG_AUTO_ENABLE); }
